...
export function RelatedVideos({ insight }) {
  ...
  async function handleInsightSelection() {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_API_URL_DEV}/youtube/related`,
      {
    ...
    dispatch(
      setCatalog({ focalpointIndex: focalpointIndex, catalog: response.data })
    );
  ...
  return <StyledTbBinaryTree2 onClick={handleInsightSelection} />;
}
